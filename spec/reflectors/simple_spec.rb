...
describe ActiveRecordAnalyzer::Reflector::Simple do
  subject(:reflector) { ActiveRecordAnalyzer::Reflector::Simple.new(User) }
  ...
  describe "#has_attribute?" do
    ...
    context "when attribute is a normal attribute" do
      it "returns false" do
        expect(reflector.has_attribute?(:name)).to be_truthy
      end
    end

    context "when attribute is a has_many" do
      it "returns true" do
        expect(reflector.has_attribute?(:pictures)).to be_falsey
      end
    end
